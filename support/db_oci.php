		...
		switch ($type) {
			case "INTEGER":
			{
				...
				if (isset($info["AUTO INCREMENT"]) && $info["AUTO INCREMENT"]) {
					if ($varbytes < 4) {
						$sql .= " NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY";
					} else {
						$sql .= " NUMBER(19,0) GENERATED BY DEFAULT ON NULL AS IDENTITY";
					}
				} else {
					if ($varbytes == 1) {
						$sql .= " NUMBER(3,0)";
...
// If that fails, this might be able to be handled by using a RunRowFilter() for SELECT queries.  Convert the column to a string.
		...
		if (isset($info["NOT NULL"]) && $info["NOT NULL"] && $type != "STRING") {
			$sql .= " NOT NULL";
		}
