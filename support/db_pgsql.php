...
class MDDB_pgsql extends MDDB {
	...
	public function GetDisplayName() {
		return MDDB::DB_Translate("PostgreSQL (via PDO)");
	}
		...
		switch ($cmd) {
			...
			case "BULK IMPORT MODE":
			{
				...
				if ($queryinfo) {
					...
					$sql = "SET synchronous_commit TO DEFAULT";
				}
	...
	protected function RunStatementFilter(&$stmt, &$filteropts) {
		if ($filteropts["mode"] == "INSERT") {
			...
			$result = new MDDB_PDO_Statement($this, $stmt, $filteropts);
			$row    = $result->NextRow();
			...
			case "SHOW CREATE TABLE":
			{
				if ($row !== false) {
					...
					while ($row2 = $result2->NextRow()) {
						if (isset($filteropts["hints"]) && isset($filteropts["hints"][$row2->column_name])) {
							$col = $filteropts["hints"][$row2->column_name];
						} else {
							$row2->data_type = str_replace(array(
								" with timezone",
								"without timezone",
								"[]"
							), "", $row2->data_type);
							...
							switch ($row2->data_type) {
								...
								case "double precision":
									...
									break;
								case "numeric":
									$col = array("DECIMAL", $row2->numeric_precision, $row2->numeric_scale);
									break;
								case "\"char\"":
									$col = array("STRING", 1, $row2->character_maximum_length, "FIXED" => true);
									break;
								case "character varying":
									$col = array("STRING", 1, $row2->character_maximum_length);
									break;
								case "text":
									$col = array("STRING", 4);
									break;
							...
							if (isset($row2->column_default)) {
								if (strtolower(substr($row2->column_default, 0, 8)) == "nextval(" && strtolower(substr($row2->column_default, - 11)) == "::regclass)") {
									$col["AUTO INCREMENT"] = true;
								} else {
									$col["DEFAULT"] = $row2->column_default;
								}
							}
						}
					...
					while ($row2 = $result2->NextRow()) {
						if ($lastindex != $row2->oid) {
							...
							if ($row2->indisprimary) {
								$type = "PRIMARY";
							} else if ($row2->indisunique) {
								$type = "UNIQUE";
							} else {
								$type = "KEY";
							}
					...
					if ($row->table_type == "LOCAL TEMPORARY") {
