...
class MDDB_sqlite extends MDDB {
	...
	public function GetDisplayName() {
		return MDDB::DB_Translate("SQLite (via PDO)");
	}
		...
		switch ($cmd) {
				...
				if (isset($queryinfo[2]) && is_array($queryinfo[2])) {
					foreach ($queryinfo[2] as $info) {
						if (strtoupper($info[0]) == "KEY" && isset($info["NAME"])) {
							$sql2   = "";
							$opts2  = array();
							$result = $this->GenerateSQL($master, $sql2, $opts2, "ADD INDEX", array(
								$queryinfo[0],
								$info
							), array(), false);
							if (!$result["success"]) {
			...
			case "DROP COLUMN":
			{
				...
				if (!$row) {
					return array(
						"success"   => false,
						"error"     => MDDB::DB_Translate("An error occurred while getting existing table information."),
						"errorcode" => "show_create_table_error"
					);
				}

				if (!isset($row->opts[1][$queryinfo[1]])) {
					return array("success" => false, "errorcode" => "skip_sql_query");
				}
				...
				if (!count($row->opts[1])) {
					return array(
						"success"   => false,
						"error"     => MDDB::DB_Translate("Table contains only one column."),
						"errorcode" => "drop_column_error"
					);
				}
			...
			case "ADD INDEX":
			{
				...
				foreach ($keyinfo[1] as $num => $field) {
					$keyinfo[1][$num] = $this->QuoteIdentifier($field);
				}

				if (!isset($keyinfo["NAME"])) {
					return array("success" => false, "errorcode" => "skip_sql_query");
				}
			...
			case "SHOW DATABASES":
			{
				...
				if (isset($queryinfo[0])) {
					$queryinfo[0] = $this->GetDBPrefix($queryinfo[0]);
				}
		...
		switch ($filteropts["mode"]) {
			case "SHOW DATABASES":
			{
				if ($row !== false) {
					...
					if (isset($filteropts["names"][$name])) {
						$fetchnext = true;
					} else {
			...
			case "SHOW TABLES":
			{
				if ($row !== false) {
					...
					if ($dbprefix == "" && strpos($name, "__") !== false) {
						$fetchnext = true;
					} else if ($dbprefix != "" && strtolower(substr($name, 0, strlen($dbprefix))) !== strtolower($dbprefix)) {
						$fetchnext = true;
					} else {
			...
			case "SHOW CREATE DATABASE":
			{
				if ($row !== false) {
					...
					if ($filteropts["output"] || $name == "" || $name !== $filteropts["queryinfo"][0]) {
						$fetchnext = true;
					} else {
						$row->name = $name;
				...
				if ($row !== false) {
					if ($filteropts["output"]) {
						...
						while ($row2 = $result2->NextRow()) {
							if (isset($filteropts["hints"]) && isset($filteropts["hints"][$row2->name])) {
								$col = $filteropts["hints"][$row2->name];
							} else {
								if (stripos($row2->type, "INT") !== false) {
									$col = array("INTEGER", 8);
									...
									if ($pos !== false) {
										...
										if ($pos2 !== false) {
											...
											if ($num > 0 && $num < 256) {
												$col = array("STRING", 1, $num);
											}
										}
									}
								} else if (stripos($row2->type, "BLOB") !== false || $row2->type === "") {
									$col = array("BINARY", 4);
								} else if (stripos($row2->type, "REAL") !== false || stripos($row2->type, "FLOA") !== false || stripos($row2->type, "DOUB") !== false) {
									$col = array("FLOAT", 8);
								} else {
									$col = array("NUMERIC");
									$pos = strpos($row2->type, "(");
								...
								if (isset($row2->dflt_value)) {
									$col["DEFAULT"] = $row2->dflt_value;
								}
								if ($row2->pk > 0) {
									$col["PRIMARY KEY"] = true;
									if ($col[0] == "INTEGER") {
										$col["AUTO INCREMENT"] = true;
									}
								}
							}
						...
						while ($row2 = $result2->NextRow()) {
							...
							if (substr($row2->name, 0, strlen($this->dbprefix . $filteropts["queryinfo"][0]) + 2) == $this->dbprefix . $filteropts["queryinfo"][0] . "__") {
								$row2->name = substr($row2->name, strlen($this->dbprefix . $filteropts["queryinfo"][0]) + 2);
							}
						...
						if ($row->tbl_type == "temp") {
							$row->opts["TEMPORARY"] = true;
						}
