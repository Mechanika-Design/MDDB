...
class MDDB_mysql extends MDDB {
	...
	public function GetDisplayName() {
		return MDDB::DB_Translate("MySQL/Maria DB (via PDO)");
	}
	...
	protected function GenerateSQL(&$master, &$sql, &$opts, $cmd, $queryinfo, $args, $subquery) {
		switch ($cmd) {
			...
			case "CREATE DATABASE":
			{
				...
				if (isset($queryinfo["COLLATE"])) {
					$sql .= " COLLATE " . $queryinfo["COLLATE"];
				}
			...
			case "CREATE TABLE":
			{
				...
				if (!isset($queryinfo["CHARACTER SET"])) {
					$queryinfo["CHARACTER SET"] = "utf8";
				}
			...
			case "ADD COLUMN":
			{
				...
				if (isset($queryinfo["FIRST"]) && $queryinfo["FIRST"]) {
					$sql .= " FIRST";
				} else if (isset($queryinfo["AFTER"])) {
					$sql .= " AFTER " . $this->QuoteIdentifier($queryinfo["AFTER"]);
				}
			...
			case "DROP INDEX":
			{
				...
				if ($type == "PRIMARY") {
					...
					$sql .= "FOREIGN KEY " . $this->QuoteIdentifier($queryinfo[2]);
				} else {
					$sql .= "KEY " . $this->QuoteIdentifier($queryinfo[2]);
				}
			...
			case "DROP TABLE":
			{
				...
				if (isset($queryinfo["TEMPORARY"]) && $queryinfo["TEMPORARY"]) {
					$cmd = "DROP TEMPORARY TABLE";
				}
				$sql = $cmd . " " . $this->QuoteIdentifier($queryinfo[0]);
				if (isset($queryinfo["RESTRICT"]) && $queryinfo["RESTRICT"]) {
					$sql .= " RESTRICT";
				} else if (isset($queryinfo["CASCADE"]) && $queryinfo["CASCADE"]) {
					$sql .= " CASCADE";
				}
			...
			{
				$sql = $cmd;
				if (isset($queryinfo[0]) && $queryinfo[0] == "") {
					unset($queryinfo[0]);
				}

				return array(
					"success"     => true,
					"filter_opts" => array("mode" => "SHOW DATABASES", "queryinfo" => $queryinfo)
				);
			}
			case "SHOW TABLES":
			{
				if (isset($queryinfo["FULL"]) && $queryinfo["FULL"]) {
					$cmd = "SHOW FULL TABLES";
				}
				$sql = $cmd;
				if (isset($queryinfo["FROM"])) {
					$sql .= " FROM " . $this->QuoteIdentifier($queryinfo["FROM"]);
				}
				if (isset($queryinfo[0]) && $queryinfo[0] == "") {
					unset($queryinfo[0]);
				}
				...
				return array(
					"success"     => true,
					"filter_opts" => array("mode" => "SHOW TABLES", "queryinfo" => $queryinfo)
				);
			}
	...
	protected function ProcessColumnDefinition($info) {
		...
		switch ($type) {
			case "INTEGER":
			{
				...
				if (isset($info["UNSIGNED"]) && $info["UNSIGNED"]) {
					$sql .= " UNSIGNED";
				}
			...
			case "DECIMAL":
			{
				$sql .= " DECIMAL";
				if (isset($info[1])) {
					$sql .= "(" . $info[1] . (isset($info[2]) ? ", " . $info[2] : "") . ")";
				}
			...
			case "STRING":
			{
				$varbytes = $info[1];
				if ($varbytes == 1) {
					if (isset($info["FIXED"]) && $info["FIXED"]) {
						$sql .= " CHAR(" . $info[2] . ")";
					} else {
						$sql .= " VARCHAR(" . $info[2] . ")";
					}
			...
			default:
				return array(
					"success"   => false,
					"error"     => MDDB::DB_Translate("Unknown column type '%s'.", $type),
					"errorcode" => "unknown_column_type"
				);
		}
		...
		if (isset($info["DEFAULT"])) {
			$sql .= " DEFAULT " . $this->Quote($info["DEFAULT"]);
		}
		if (isset($info["PRIMARY KEY"]) && $info["PRIMARY KEY"]) {
			$sql .= " PRIMARY KEY";
		}
		if (isset($info["UNIQUE KEY"]) && $info["UNIQUE KEY"]) {
			$sql .= " UNIQUE KEY";
		}
		...
		if (isset($info["COMMENT"])) {
			$sql .= " COMMENT " . $this->Quote($info["COMMENT"]);
		}
		if (isset($info["REFERENCES"])) {
			$sql .= " REFERENCES " . $this->ProcessReferenceDefinition($info["REFERENCES"]);
		}
	...
	protected function ProcessKeyDefinition($info) {
		...
		switch ($type) {
			...
			default:
				return array(
					"success"   => false,
					"error"     => MDDB::DB_Translate("Unknown key type '%s'.", $type),
					"errorcode" => "unknown_key_type"
				);;
		}
		...
		switch ($filteropts["mode"]) {
					...
					if ($pos !== false) {
						...
						if ($pos2 !== false) {
							...
							if ($pos !== false) {
								...
								$pos2 = strpos($str, " ", $pos);
								if ($pos2 !== false) {
									$opts["CHARACTER SET"] = substr($str, $pos, $pos2 - $pos);
								}
							}
							...
							if ($pos !== false) {
								...
								$pos2 = strpos($str, " ", $pos);
								if ($pos2 !== false) {
									$opts["COLLATE"] = substr($str, $pos, $pos2 - $pos);
								}
							}
						}
					}
			...
			case "SHOW CREATE TABLE":
			{
				if ($row !== false) {
					...
					if (strtoupper(substr($str, 0, 23)) == "CREATE TEMPORARY TABLE ") {
						$opts["TEMPORARY"] = true;
					}
							...
							while ($str != "" && $str{0} != ")") {
								...
								if ($ident || !isset($keytypes[strtoupper($id)])) {
									...
									switch (strtoupper($type)) {
										...
										{
											$opts2 = array("INTEGER");
											if (!count($typeopts) || $typeopts[0] <= 8) {
												$opts2[] = 1;
											} else if ($typeopts[0] <= 16) {
												$opts2[] = 2;
											} else if ($typeopts[0] <= 24) {
												$opts2[] = 3;
											} else if ($typeopts[0] <= 32) {
												$opts2[] = 4;
											} else {
												$opts2[] = 8;
											}
										...
										case "TINYINT":
											$opts2  = array("INTEGER", 1);
											$extras = array("UNSIGNED" => "keep", "ZEROFILL" => "ignore");
											break;
										case "SMALLINT":
											$opts2  = array("INTEGER", 2);
											$extras = array("UNSIGNED" => "keep", "ZEROFILL" => "ignore");
											break;
										case "MEDIUMINT":
											$opts2  = array("INTEGER", 3);
											$extras = array("UNSIGNED" => "keep", "ZEROFILL" => "ignore");
											break;
										case "INT":
										case "INTEGER":
											$opts2  = array("INTEGER", 4);
											$extras = array("UNSIGNED" => "keep", "ZEROFILL" => "ignore");
											break;
										case "BIGINT":
											$opts2  = array("INTEGER", 8);
											$extras = array("UNSIGNED" => "keep", "ZEROFILL" => "ignore");
											break;
										...
										case "REAL":
											$opts2  = array("FLOAT", 8);
											$extras = array("UNSIGNED" => "ignore", "ZEROFILL" => "ignore");
											break;
										case "FLOAT":
											$opts2  = array("FLOAT", 4);
											$extras = array("UNSIGNED" => "ignore", "ZEROFILL" => "ignore");
										...
										case "NUMERIC":
											$opts2[] = array(
												"DECIMAL",
												(count($typeopts) ? $typeopts[0] : 10),
												(count($typeopts) > 1 ? $typeopts[1] : 0)
											);
											$extras  = array("UNSIGNED" => "ignore", "ZEROFILL" => "ignore");
											break;
										...
										case "CHAR":
											$opts2  = array(
												"STRING",
												1,
												(count($typeopts) ? $typeopts[0] : 255),
												"FIXED" => true
											);
											$extras = array(
												"CHARACTER SET" => "ignore_with_opt",
												"COLLATE"       => "ignore_with_opt"
											);
											break;
										case "VARCHAR":
											$opts2  = array("STRING", ($typeopts[0] > 255 ? 2 : 1), $typeopts[0]);
											$extras = array(
												"CHARACTER SET" => "ignore_with_opt",
												"COLLATE"       => "ignore_with_opt"
											);
										...
										case "BINARY":
											...
											break;
										case "VARBINARY":
											$opts2 = array("BINARY", ($typeopts[0] > 255 ? 2 : 1), $typeopts[0]);
											break;
										case "TINYBLOB":
											$opts2 = array("BINARY", 1, 255);
											break;
										case "BLOB":
											$opts2 = array("BINARY", 2);
											break;
										case "MEDIUMBLOB":
											$opts2 = array("BINARY", 3);
											break;
										case "LONGBLOB":
											$opts2 = array("BINARY", 4);
											break;
									...
									do {
										...
										foreach ($extras as $extra => $rule) {
											if (strtoupper(substr($str, 0, strlen($extra))) == $extra) {
												...
												if ($rule == "keep") {
													$opts2[$extra] = true;
												} else if ($rule == "ignore_with_opt") {
													$this->ExtractIdentifier($str, $ident);
												}
											...
											if (strtoupper(substr($str, 0, strlen($extra))) == $extra) {
												...
												switch ($extra) {
													...
													case "NULL":
														break;
													case "DEFAULT":
														$opts2["DEFAULT"] = $this->ExtractAndUnescapeValue($str);
														break;
													case "AUTO_INCREMENT":
														$opts2["AUTO INCREMENT"] = true;
														break;
													case "UNIQUE KEY":
													case "UNIQUE":
														$opts2["UNIQUE KEY"] = true;
														break;
													case "PRIMARY KEY":
													case "PRIMARY":
														$opts2["PRIMARY KEY"] = true;
														break;
													case "COMMENT":
														$opts2["COMMENT"] = $this->ExtractAndUnescapeValue($str);
														break;
													case "REFERENCES":
														$opts2["REFERENCES"] = $this->ExtractReferencesInfo($str);
														break;
												}
											}
										}
									...
									if (isset($filteropts["hints"]) && isset($filteropts["hints"][$id])) {
										$opts2 = $filteropts["hints"][$id];
									}
										...
										if (strtoupper(substr($str, 0, strlen($extra))) == $extra) {
											...
											switch ($extra) {
												case "PRIMARY KEY":
												{
													...
													while ($str != "" && $str{0} != "(") {
														$id = $this->ExtractIdentifier($str, $ident);
														if (!$ident && strtoupper($id) == "USING") {
															$opts2["USING"] = $this->ExtractIdentifier($str, $ident);
														}
													}
												...
												case "FULLTEXT":
												{
													$opts2[] = "FULLTEXT";
													while ($str != "" && $str{0} != "(") {
														$opts2["NAME"] = $this->ExtractIdentifier($str, $ident);
													}
													$opts2[] = $this->ExtractMultipleIdentifiers($str);
								...
								foreach ($extras as $extra) {
									if (strtoupper(substr($str, 0, strlen($extra))) == $extra) {
										...
										switch ($extra) {
											case "ENGINE":
											case "TYPE":
												$opts["ENGINE"] = $this->ExtractAndUnescapeValue($str);
												break;
	...
	private function ExtractIdentifier(&$str, &$ident) {
		...
		if ($str{0} == "`") {
			...
			if (!preg_match('/[^A-Za-z0-9_]/', $str, $matches, PREG_OFFSET_CAPTURE)) {
				$matches = array(array($str, strlen($str)));
			}
		...
		do {
			...
			foreach ($extras as $extra) {
				if (strtoupper(substr($str, 0, strlen($extra))) == $extra) {
					...
					switch ($extra) {
						case "MATCH FULL":
							$result["MATCH FULL"] = true;
							break;
						case "MATCH PARTIAL":
							$result["MATCH PARTIAL"] = true;
							break;
						case "MATCH SIMPLE":
							$result["MATCH SIMPLE"] = true;
							break;
