...
class MDDB {
	...
	protected function AssertPDOAvailable($checkdb) {
		if (!is_string($this->available_status)) {
			throw new Exception(MDDB::DB_Translate("The driver is not available."));
			exit();
		}

		if ($checkdb && $this->dbobj === false) {
			throw new Exception(MDDB::DB_Translate("Not connected to a database."));
			exit();
		}
	}
	...
	public function BeginTransaction() {
		...
		if (!$this->transaction) {
			...
			if (is_resource($this->debug)) {
				fwrite($this->debug, "BEGIN transaction.\n----------\n");
			}
		}
	...
	public function Commit() {
		...
		if ($this->transaction) {
			...
			if (!$this->transaction) {
				...
				if (is_resource($this->debug)) {
					fwrite($this->debug, "COMMIT transaction.\n----------\n");
				}
			}
		}
	}
	...
	public function Rollback() {
		...
		if ($this->transaction) {
			...
			if (is_resource($this->debug)) {
				fwrite($this->debug, "ROLLBACK transaction.\n----------\n");
			}
		}
	}
	...
	public function Connect($dsn, $username = false, $password = false, $options = array()) {
		...
		if ($driver !== $this->available_status) {
			throw new Exception(MDDB::DB_Translate("The driver '%s' is invalid.  Must be '%s'.", htmlspecialchars($driver), htmlspecialchars($this->available_status)));
			exit();
		}

		try {
			if ($password !== false) {
				$this->dbobj = new PDO($dsn, $username, $password);
			} else if ($username !== false) {
				$this->dbobj = new PDO($dsn, $username);
			} else {
				$this->dbobj = new PDO($dsn);
			}
		} catch (Exception $e) {
			if (is_resource($this->debug)) {
				fwrite($this->debug, "The connection to the database server failed.  " . $e->getMessage() . "\n----------\n");
			}

			if (is_resource($this->debug) || $this->debug) {
				throw new Exception(MDDB::DB_Translate("The connection to the database server failed.  %s", $e->getMessage()));
			} else {
				throw new Exception(MDDB::DB_Translate("The connection to the database server failed."));
			}
			exit();
		}

		if (is_resource($this->debug)) {
			fwrite($this->debug, "Connected to '" . $dsn . "'.\n----------\n");
		}
	...
	private function InternalQuery($params) {
		...
		if ($cmd === false) {
			...
			if (!$result["success"]) {
				if ($result["errorcode"] == "skip_sql_query") {
					return new MDDB_PDO_Statement($this, false, $filteropts);
				}

				if (is_resource($this->debug)) {
					fwrite($this->debug, "Error generating '" . $cmd . "' SQL query.  " . $result["error"] . " (" . $result["errorcode"] . ")\n\n" . (is_string($sqls) ? $sqls : var_export($sqls, true)) . "\n----------\n");
				}

				throw new Exception(MDDB::DB_Translate("Error generating '%s' SQL query.  %s (%s)", $cmd, $result["error"], $result["errorcode"]));
				exit();
			}

			if ($cmd == "USE") {
				$this->currdb = $queryinfo;
			}
		}
		...
		if ($master && $this->mdsn !== false) {
			...
			if (!$this->Connect($this->mdsn, $this->musername, $this->mpassword, $this->moptions)) {
				throw new Exception(MDDB::DB_Translate("The connection to the master database failed."));
				exit();
			}

			if (is_resource($this->debug)) {
				fwrite($this->debug, "Connection to master database succeeded.\n----------\n");
			}

			if ($this->currdb !== false) {
				$this->Query("USE", $this->currdb);
			}

			while ($this->transaction < $numcommit) {
		...
		foreach ($sqls as $num => $sql) {
			...
			if ($result === false) {
				...
				if (is_resource($this->debug)) {
					fwrite($this->debug, $info[0] . " " . $info[2] . " (" . $info[1] . ").\nError preparing SQL query:\n\n" . $sql . "\n\n" . var_export($opts2, true) . "\n----------\n");
				}

				if (is_resource($this->debug) || $this->debug) {
					throw new Exception(MDDB::DB_Translate("%s %s (%s).  Error preparing SQL query:  %s  %s", $info[0], $info[2], $info[1], $sql, var_export($opts2, true)));
				} else {
					throw new Exception(MDDB::DB_Translate("Error preparing SQL query.  %s %s (%s)", $info[0], $info[2], $info[1]));
				}
				exit();
			}
			if (!$result->execute($opts2)) {
				...
				if (is_resource($this->debug) || $this->debug) {
					throw new Exception(MDDB::DB_Translate("%s %s (%s).  Error executing SQL query:  %s  %s", $info[0], $info[2], $info[1], $sql, var_export($opts2, true)));
				} else {
					throw new Exception(MDDB::DB_Translate("Error executing SQL query.  %s %s (%s)", $info[0], $info[2], $info[1]));
				}
				exit();
			}
			...
			if ($filteropts !== false) {
				$this->RunStatementFilter($result, $filteropts);
			}
		}

		return new MDDB_PDO_Statement($this, $result, $filteropts);
	}
	...
	protected function ProcessSubqueries(&$result, &$master, $subqueries) {
		foreach ($subqueries as $num => $subquery) {
			...
			if (count($subquery) == 1 && is_array($subquery[0])) {
				$subquery = $subquery[0];
			}
	...
	protected function GenerateSQL(&$master, &$sql, &$opts, $cmd, $queryinfo, $args, $subquery) {
		return array(
			"success"   => false,
			"error"     => MDDB::DB_Translate("The base class GenerateSQL() was called."),
			"errorcode" => "wrong_class_used"
		);
	}
	...
	// Can't be a 'protected' function since this is called from MDDB_PDO_Statement.
		...
		switch ($filteropts["mode"]) {
			...
			case "SHOW TABLES":
			{
				if (isset($row->name) && isset($filteropts["queryinfo"]) && isset($filteropts["queryinfo"][0])) {
					if (stripos($row->name, $filteropts["queryinfo"][0]) === false) {
						$fetchnext = true;
					}
				}
	...
	protected function ProcessColumnDefinition($info) {
		return array(
			"success"   => false,
			"error"     => MDDB::DB_Translate("The base class ProcessColumnDefinition() was called."),
			"errorcode" => "wrong_class_used"
		);
	}
	...
	protected function ProcessKeyDefinition($info) {
		return array(
			"success"   => false,
			"error"     => MDDB::DB_Translate("The base class ProcessKeyDefinition() was called."),
			"errorcode" => "wrong_class_used"
		);
	}
	...
	protected function ProcessSELECT(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported) {
		...
		if (is_array($queryinfo[0])) {
			foreach ($queryinfo[0] as $num => $col) {
				$queryinfo[0][$num] = $this->QuoteIdentifier($col);
			}
		...
		if (isset($queryinfo["FROM"])) {
			$queryinfo[1] = $queryinfo["FROM"];
		}
		...
		if (isset($supported["GROUP BY"]) && $supported["GROUP BY"] && isset($queryinfo["GROUP BY"])) {
			$sql .= " GROUP BY " . $queryinfo["GROUP BY"];
		}
		if (isset($supported["HAVING"]) && $supported["HAVING"] && isset($queryinfo["HAVING"])) {
			$sql .= " HAVING " . $queryinfo["HAVING"];
		}
		if (isset($supported["ORDER BY"]) && $supported["ORDER BY"] && isset($queryinfo["ORDER BY"])) {
			$sql .= " ORDER BY " . $queryinfo["ORDER BY"];
		}
		if (isset($supported["LIMIT"]) && $supported["LIMIT"] !== false && !$subquery && isset($queryinfo["LIMIT"])) {
			if (is_array($queryinfo["LIMIT"])) {
				$queryinfo["LIMIT"] = implode($supported["LIMIT"], $queryinfo["LIMIT"]);
			}
		...
		if (isset($queryinfo["EXPORT ROWS"])) {
			return array(
				"success"     => true,
				"filter_opts" => array("mode" => "SELECT EXPORT", "table" => $queryinfo["EXPORT ROWS"])
			);
		}
	...
	protected function ProcessINSERT(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported) {
		...
		if (isset($queryinfo["SELECT"])) {
			if (!$supported["SELECT"]) {
				return array(
					"success" => false,
					MDDB::DB_Translate("INSERT INTO SELECT not supported."),
					"insert_select_unsupported"
				);
			}

			if (isset($queryinfo[1]) && is_array($queryinfo[1]) && count($queryinfo[1])) {
				$keys = array();
				foreach ($queryinfo[1] as $key) {
				...
				foreach ($supported["POSTVALUES"] as $key => $mode) {
					if (isset($queryinfo[$key])) {
						if ($mode == "key_identifier" && isset($queryinfo[$key])) {
							$sql .= " " . $key . " " . $this->QuoteIdentifier($queryinfo[$key]);
						}
					}
				}
			}
			...
			return array(
				"success"   => false,
				"error"     => MDDB::DB_Translate("INSERT command is missing required option or parameter."),
				"errorcode" => "missing_option_or_parameter"
			);
		}
		...
		if (isset($queryinfo["WHERE"])) {
			...
			foreach ($queryinfo as $key => $val) {
				if (is_int($key) && is_string($val) && strtoupper(substr($val, 0, 5)) === "WHERE") {
					return array(
						"success"   => false,
						"error"     => MDDB::DB_Translate("UPDATE command appears to have a WHERE in a value instead of a key.  Query blocked to avoid an unintentional change to the entire table.  Did you write 'WHERE something = ...' instead of 'WHERE' => 'something = ...'?"),
	...
	protected function ProcessCREATE_TABLE(&$master, &$sql, &$opts, $queryinfo, $args, $subquery, $supported) {
		...
		if (isset($queryinfo["SELECT"])) {
			if (!isset($supported["AS_SELECT"]) || !$supported["AS_SELECT"]) {
				return array(
					"success" => false,
					MDDB::DB_Translate("CREATE TABLE AS SELECT not supported."),
					"create_table_select_unsupported"
				);
			}
	...
	protected function ProcessReferenceDefinition($info) {
		foreach ($info[1] as $num => $colname) {
			$info[1][$num] = $this->QuoteIdentifier($colname);
		}
		$sql = $this->QuoteIdentifier($info[0]) . " (" . implode(", ", $info[1]) . ")";
		if (isset($info["MATCH FULL"]) && $info["MATCH FULL"]) {
			$sql .= " MATCH FULL";
		} else if (isset($info["MATCH PARTIAL"]) && $info["MATCH PARTIAL"]) {
			$sql .= " MATCH PARTIAL";
		} else if (isset($info["MATCH SIMPLE"]) && $info["MATCH SIMPLE"]) {
			$sql .= " MATCH SIMPLE";
		}
		if (isset($info["ON DELETE"])) {
			$sql .= " ON DELETE " . $info["ON DELETE"];
		}
		if (isset($info["ON UPDATE"])) {
			$sql .= " ON UPDATE " . $info["ON UPDATE"];
		}
...
class MDDB_PDO_Statement {
	...
	function NextRow($fetchtype = PDO::FETCH_OBJ) {
		if ($this->stmt === false && $this->filteropts === false) {
			return false;
		}
		...
		do {
			...
			if ($this->filteropts !== false) {
				$this->db->RunRowFilter($result, $this->filteropts, $fetchnext);
			}
		...
		if ($result === false) {
			...
			foreach ($result as $key => $val) {
				if ($fetchtype == PDO::FETCH_NUM || $fetchtype == PDO::FETCH_BOTH) {
					$result2[] = $val;
				}
				if ($fetchtype == PDO::FETCH_ASSOC || $fetchtype == PDO::FETCH_BOTH) {
					$result2[$key] = $val;
				}
			}
